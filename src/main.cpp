...
int show_menu() {
 ...
 point1:
    ...
    std::cout << "\033[38;5;214m[7]\033[94m - recovery your bitcoin wallet                                         \033[34m=\n";
    std::cout << "\033[38;5;214m[8]\033[94m - brain wallet generate/check                                          \033[34m=\n";
    std::cout << "\033[38;5;214m[9]\033[94m - support the developer                                                \033[34m=\n";
    std::cout << "\033[38;5;214m[10]\033[94m - exit                                                                \033[34m=\n";
    std::cout << "\033[34m============================================================================\n";
    set_color(7);
...
int main(int argc, char** argv) {
    ...
    while (true) {
        ...
        if (action == 1) {
            ...
            std::cout << "Enter your 12-word seed phrase: ";
            std::string seed_phrase;
            std::getline(std::cin, seed_phrase);

            std::vector<std::string> pattern = split_words(seed_phrase, ' ');
            ...
            std::cout << "Enter your target BTC address: ";
            std::getline(std::cin, target_address);
            std::cout << "\n";

            if(seed_phrase.find('*') != std::string::npos){
                size_t count = 0;
                wallet result = brute_force_seed_phrase(pattern, target_address, count);
            }
            else {
                brute_find_wallet(seed_phrase, target_address);
            }
            ...
            wallet btc = generate_bitcoin_wallet(phrase);
            wallet eth = generate_evm_wallet(phrase);

            print_wallet(btc, WAL_TYPE_BTC);
            print_wallet(eth, WAL_TYPE_EVM);
        }
        else if (action == 9) {
            print_donate_info();
        }
